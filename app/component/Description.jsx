import Image from 'next/image';
import React from 'react'
import { FiDownload } from "react-icons/fi";

const Description = () => {
  return (
    <div className="flex flex-col sm:flex-row items-center sm:items-start mb-8">
      <div className="w-24 h-24 rounded-full overflow-hidden mr-0 sm:mr-6 mb-4 sm:mb-0 flex-shrink-0">
        <Image
          src="/bibek-profile-pic.jpg"
          alt="Bibek Ray"
          width={96}
          height={96}
          className="object-cover w-full h-full"
        />
      </div>
      <div>
        <h1 className="text-3xl font-bold">Bibek Ray</h1>
        <p className="text-gray-400 text-xl">Kathmandu, Nepal</p>
        {/* <button className="mt-2 border-1 border-gray-400 hover:border-gray-50 text-sm text-white py-1 px-3 rounded-full flex items-center transition-colors duration-100">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 87.3 78"
            width="1em"
            height="1em"
            
          >
            <path
              fill="#0066da"
              d="m6.6 66.85 3.85 6.65c.8 1.4 1.95 2.5 3.3 3.3L27.5 53H0c0 1.55.4 3.1 1.2 4.5z"
            />
            <path
              fill="#00ac47"
              d="M43.65 25 29.9 1.2c-1.35.8-2.5 1.9-3.3 3.3l-25.4 44A9.06 9.06 0 0 0 0 53h27.5z"
            />
            <path
              fill="#ea4335"
              d="M73.55 76.8c1.35-.8 2.5-1.9 3.3-3.3l1.6-2.75L86.1 57.5c.8-1.4 1.2-2.95 1.2-4.5H59.798l5.852 11.5z"
            />
            <path
              fill="#00832d"
              d="M43.65 25 57.4 1.2C56.05.4 54.5 0 52.9 0H34.4c-1.6 0-3.15.45-4.5 1.2z"
            />
            <path
              fill="#2684fc"
              d="M59.8 53H27.5L13.75 76.8c1.35.8 2.9 1.2 4.5 1.2h50.8c1.6 0 3.15-.45 4.5-1.2z"
            />
            <path
              fill="#ffba00"
              d="m73.4 26.5-12.7-22c-.8-1.4-1.95-2.5-3.3-3.3L43.65 25 59.8 53h27.45c0-1.55-.4-3.1-1.2-4.5z"
            />
          </svg>
          <span className="ml-2">View Resume</span>
        </button> */}
      </div>
    </div>
  );
}

export default Description